...
function AppContent() {
  useEffect(() => {
    ...
    const initializeNotifications = async () => {
      try {
        ...
        if (hasPermission) {
          ...
          // ⭐ NUEVO: Verificar préstamos al iniciar la app
          console.log('Verificando préstamos al iniciar...');
    ...
    // ⭐ NUEVO: Verificar cuando la app vuelve del segundo plano
    const { AppState } = require('react-native');
    const appStateSubscription = AppState.addEventListener('change', async (nextAppState: string) => {
      if (nextAppState === 'active') {
        console.log('App volvió a primer plano, verificando préstamos...');
        try {
          const { LoansService } = require('./src/services/loansService');
          await LoansService.actualizarEstadosPrestamos();
        } catch (error) {
